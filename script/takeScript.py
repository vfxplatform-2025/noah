...
def takeItem(item, itemdir, extType, scale, renderLayerName="", tabType=""):
    ...
    if ":" in item:
        ...
        if extType == "usd":
            ...
            fileinfo[ "%s:%s" %(itemName,assetName,) ] = "%s/%s_scale%s.%s" %(itemNewDir, fileName, scale, extType)
        ...
        if extType == "usd":
            ...
            fileinfo[ "%s" %itemName ] = "%s/%s_scale%s.%s" %(itemNewDir, fileName, scale, extType)

    if extType == "usd":
        usd_file = "%s/%s.%s" %(itemNewDir, fileName, extType)
        if os.path.isfile(usd_file):
            print("delete %s" % usd_file)
            os.system("rm -rf %s" % usd_file)
    else:
        scale_file = "%s/%s_scale%s.%s" %(itemNewDir, fileName, scale, extType)
        if os.path.isfile(scale_file):
            print("delete %s" % scale_file)
            os.system("rm -rf %s" % scale_file)
