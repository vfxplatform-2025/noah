...
def itemExport( fileName, itemdir,  item):
    ...
    for item in fileNameList:
        ...
        mel.eval( 'AbcExport -j "-frameRange %d %d -uvWrite -writeUVSets -eulerFilter -writeVisibility -worldSpace -dataFormat ogawa -root %s -file %s/%s.abc" ' \
                  %( (startF), (endF), cmds.ls('high_grp', l=1)[0], itemNewDir, item) )

        # /show/ORV/assets/aircraft/test/model/wip/data/alembic/test_model_v01
        usd_export.UsdExporter(itemNewDir, item)
        usd_model_pub.UsdModelPub(f"{itemdir}/{fileName}", fileName)
        # ('/show/ORV/assets/aircraft/test/model/pub/test_model_v02', 'test_model_v02')
