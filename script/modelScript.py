...
def modelExport(fileName, itemdir, itemName=None):  # exort 5
    ...
    fileinfo[objName] = f"{itemdir}/{fileName}/{fileName}.mb"

    pubMod(itemName, f"{itemdir}/{fileName}/{fileName}.mb")
    cmds.file(f"{itemdir}/{fileName}/{fileName}.mb", force=1, options="v=0;", typ="mayaBinary", pr=1, es=1, ch=0, chn=0,
              exp=0, con=0)
    print(f'Successfully moved to the pub folder. >> {itemdir}/{fileName}/{fileName}.mb')
    return fileinfo
...
#     name = "%s_Grp" %itemName
    ...
    if cmds.ls(itemName):  # obj in ...
        print("There is the same object !! Finding available number...")
        ...
        if ":" in itemName:
            ...
            while True:
                ...
                if not cmds.ls(newItemName):
                    ...
                    print("Using new name: %s" % itemName)
                    break
                num += 1
        else:
            # 네임스페이스가 없는 경우
