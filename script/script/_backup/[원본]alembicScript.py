...
def alembicExport( fileList , itemdir,  item, num, step_num, frameType="FrameRange", typeV="", hiddenV=[]):
    ...
    if typeV == "mergeObjectSpace":
        print("aaaaa")
        mel.eval(
            'AbcExport -j "-frameRange %d %d -uvWrite -writeUVSets -eulerFilter -writeVisibility -step %s %s -dataFormat ogawa -root %s -file %s/%s.abc" ' \
        ...
        print("bbbbb")
        mel.eval(
            'AbcExport -j "-frameRange %d %d -uvWrite -writeUVSets -eulerFilter -writeVisibility -step %s %s -worldSpace -dataFormat ogawa -root %s -file %s/%s.abc" ' \
        ...
        print('AbcExport -j "-frameRange %d %d -uvWrite -writeUVSets -eulerFilter -writeVisibility -step %s %s -worldSpace -dataFormat ogawa -root %s -file %s/%s.abc" ' \
        % (startF, endF, step_num, abcAttrOpt, cmds.ls(item, l=1)[0], itemNewDir, fileName))
        if hiddenV:
