...
from . import fix_namespace_consistency
imp.reload(fix_namespace_consistency)

import set_Alembic_metadata
import usd_export
...
def alembicExport( fileList , itemdir,  item, num, step_num, frameType="FrameRange", typeV="", hiddenV=[]):
    ...
    if ":" in item:
        ...
        fileinfo[ "%s:%s" %(itemName,assetName,) ] = "%s/%s.abc" %(itemNewDir,  fileName)
        ...
        fileName = item
        itemNewDir = "%s/%s" % (itemdir, itemName )
        fileinfo[ "%s" %itemName ] = "%s/%s.abc" %(itemNewDir, fileName)
        print('assetName2 :', assetName)
        print('itemName2 :', itemName)
        ...
        if "_crowd_Grp" in item:
            ...
            with open("{}/{}.json".format(itemNewDir, fileName), 'w') as namespace_data:
                json.dump(namespace_dic, namespace_data, indent=4)

    if os.path.isfile( "%s/%s.abc" %(itemNewDir, fileName) ):
        print("delete %s/%s.abc" %(itemNewDir, fileName))
        os.system("rm -rf %s/%s.abc" %(itemNewDir, fileName) )
    ...
    if typeV == "mergeObjectSpace":
        print("aaaaa")
        mel.eval(
            'AbcExport -j "-frameRange %d %d -attr horizontalPan -attr verticalPan -attr zoom -attr horizontalFilmAperture -uvWrite -writeUVSets -eulerFilter -writeVisibility -step %s %s -dataFormat ogawa -root %s -file %s/%s.abc" ' \
            % (startF, endF, step_num, abcAttrOpt, cmds.ls(item.replace("_mergeObjectSpace", ""), l=1)[0], itemNewDir, fileName))
        ...
        print("bbbbb")
        mel.eval(
            'AbcExport -j "-frameRange %d %d -attr horizontalPan -attr verticalPan -attr zoom -attr horizontalFilmAperture -uvWrite -writeUVSets -eulerFilter -writeVisibility -step %s %s -worldSpace -dataFormat ogawa -root %s -file %s/%s.abc" ' \
            % (startF, endF, step_num, abcAttrOpt, cmds.ls(item.replace("_mergeWorldSpace", ""), l=1)[0], itemNewDir, fileName))
        ...
        mel.eval(
            'AbcExport -j "-frameRange %d %d -attr horizontalPan -attr verticalPan -attr zoom -attr horizontalFilmAperture -uvWrite -writeUVSets -eulerFilter -writeVisibility -step %s %s -worldSpace -dataFormat ogawa -root %s -file %s/%s.abc" ' \
                  %( startF, endF, step_num, abcAttrOpt, cmds.ls(item, l=1)[0], itemNewDir,fileName) )

        # >>>>>>>>>>>>>>>>>>>>>> jiho Editing!!!!!!! <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        #exportFilmOffset.checkOffset(item)

        print('AbcExport -j "-frameRange %d %d -attr horizontalPan -attr verticalPan -attr zoom -attr horizontalFilmAperture -uvWrite -writeUVSets -eulerFilter -writeVisibility -step %s %s -worldSpace -dataFormat ogawa -root %s -file %s/%s.abc" ' \
        % (startF, endF, step_num, abcAttrOpt, cmds.ls(item, l=1)[0], itemNewDir, fileName))
        if hiddenV:
            cmds.showHidden(hiddenV)

    #set_Alembic_metadata.run_set_alembic_metadata(itemNewDir + '/' + fileName + '.abc', scale)
...
def alembicImportFile(item, itemDir, rootType, type, lod="None"):
    ...
    if type == "alembic":  # cherryBranch9_cherryBranch.abc
        print("=== alembicScript DEBUG ===")
        print("Original item: %s" % item)
        print("About to call nameSpaceCheck...")
        
        namecheck_result = utilScript.nameSpaceCheck(item)
        print("nameSpaceCheck result: %s" % namecheck_result)
        
        if namecheck_result is False:
            print("Import cancelled by user")
            return None  # Cancel 시 import 중단
        elif namecheck_result != True:
            # 새로운 이름이 리턴된 경우 (Cancel 후 자동 넘버링)
            item = namecheck_result
            print("Updated item name to: %s" % item)
        ...
        if "_crowd_Grp" in item:
            ...
            if ":" in item:
                ...
                # Import with namespace using cmds.file
                print("DEBUG: Importing with namespace: %s, item: %s" % (namespace, item))
                print("DEBUG: About to import file: %s" % itemDir)
                print("DEBUG: Namespaces before import: %s" % cmds.namespaceInfo(listOnlyNamespaces=True))
                
                # Import with namespace using cmds.file
                # mergeNamespacesOnClash=True로 변경하여 같은 네임스페이스에 병합
                ...
                print("DEBUG: Namespaces after import: %s" % cmds.namespaceInfo(listOnlyNamespaces=True))
                print("DEBUG: Objects in target namespace: %s" % cmds.ls(namespace + ":*"))
                # print ">" * 10, returnList
            else:
            ...
            if returnList:
                ...
                if topGroup:
                    ...
                    # If no top group found, try to use the first returned node
                    tranName = returnList[0] if isinstance(returnList[0], str) else None
            else:
                tranName = None
            ...
            if tranName:
                try:
                    if ("_geo_Grp" not in tranName) and ("_geo_grp" not in tranName) and not cmds.listRelatives(tranName, p=1,
                        ...
                        print("okok")
                        # 이미 네임스페이스가 있는 경우 그대로 사용
                        cmds.group(tranName, n=item)
                except:
        ...
        if namecheck_result is False:
            ...
            # 새로운 이름이 리턴된 경우 (Cancel 후 자동 넘버링)
            item = namecheck_result
            print("Updated proxy item name to: %s" % item)
        # pluginCheck()
