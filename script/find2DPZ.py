...
#            return
        ...
        if zoom_enableKey != None or panx_enableKey != None or pany_enableKey != None:
            print("---------------------------------------------------")
            print('>>>> 2DPZ parm has key!!! & run find2DPZ!!! <<<<')
            cmds.refresh(su=1)
            cmds.bakeResults(cam_name[0]+'.pan', t=(start_frame, end_frame), simulation=True)
            cmds.bakeResults(cam_name[0]+'.zoom', t=(start_frame, end_frame), simulation=True)
            cmds.refresh(su=0)
