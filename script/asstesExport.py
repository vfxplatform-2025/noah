...
class publishWindow(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        check_envsurvey = ui_mb_path.split('/')[4]
...
class assetsView(QtWidgets.QWidget):
    ...
    def setTypeName(self, name=None):
        ...
        # set QStackedWidget
        if APPNAME == "clarisse":
            type = {"lookdev": 0, "digienv": 1}
            if "%s" % name in type:
                self.optStackedWidget_2.setCurrentIndex(type[name])
            else:
                self.optStackedWidget_2.setCurrentIndex(0)
        else:
        ...
        if "/rig/" in self.pubFileList['pubDir'] or "/model/" in self.pubFileList['pubDir']:
            ...
            if 'survey' in self.mb_path:
                ...
                for z in self.all_diff_list:
                    if self.savefileName.split('_')[-1] in z:
            ...
            self.plugin_name = "objExport.so"  # 플러그인 파일명. 다른 운영 체제에서는 확장자가 달라질 수 있습니다.
            ...
            if len(self.sel_obj) == 0:
                ...
                cmds.file(self.jiho_objPath, force=True,
                    ...
                    if checkBoxItem.isChecked() == True:
                        ...
                        for objKey in objList:
                            if not "mergeObjectSpace" in objKey:
                                ...
                                file_path = str(objList[objKey])

                                if check_type == "Camera":
                                    file_path = file_path.replace(".py", ".abc")
                                mainScriptLinux.importItem(check_type, file_path, ref_check, ab_check)
        else:
            # get take
